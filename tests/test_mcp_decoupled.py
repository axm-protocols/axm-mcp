...
class TestNoAxmImports:
    ...
    def test_mcp_app_no_axm_import(self) -> None:
        ...
        mcp_app_path = (
            Path(__file__).parent.parent / "src" / "axm_mcp" / "mcp_app.py"
        )
    ...
    def test_init_no_axm_import(self) -> None:
        ...
        init_path = (
            Path(__file__).parent.parent / "src" / "axm_mcp" / "__init__.py"
        )
...
class TestNoConfigure:
    ...
    def test_no_configure(self) -> None:
        ...
        source = mcp_app_path.read_text()
        assert (
            "def configure(" not in source
        ), "configure() is still in mcp_app.py"
...
class TestServerPackageRemoved:
    ...
    def test_no_server_package(self) -> None:
        server_dir = (
            Path(__file__).parent.parent / "src" / "axm_mcp" / "server"
        )
        assert not server_dir.exists(), "server/ package still exists"


class TestPyprojectNoDep:
    """pyproject.toml must NOT list axm as a hard dependency."""

    def test_axm_not_in_dependencies(self) -> None:
        pyproject_path = Path(__file__).parent.parent / "pyproject.toml"
        content = pyproject_path.read_text()

        # Parse to find dependencies section
        import tomllib
        ...
        for dep in deps:
            # "axm" alone or "axm>=..." but NOT "axm-bib" or "axm-formal"
            raw = dep.split(">")[0].split("<")[0]
            dep_name = raw.split("=")[0].split("[")[0].strip()
            assert dep_name != "axm", f"axm is still a hard dependency: {dep}"
